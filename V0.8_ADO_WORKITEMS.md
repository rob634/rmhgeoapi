...
**Created**: 30 JAN 2026
**Revised**: 31 JAN 2026 (Robert's feedback applied)
...
|-------------|-------------|
| **OGC Standards** | All APIs implement OGC specifications (see Standards Compliance above) |
| **Zero Secrets** | Azure Entra ID + Managed Identity only. No passwords, connection strings, or API keys in code or config |
| **Horizontally Scalable** | Containerized workloads scale out automatically under load |
| **Stateless Workers** | All processing nodes are stateless; state lives in PostgreSQL and Azure Storage |
| **Queue-Based Orchestration** | Async job processing via Azure Service Bus; no synchronous long-running requests |
| **Infrastructure as Code** | All Azure resources deployable via ARM/Bicep; no portal-only configuration |

---

## FEATURE 1: Vector Data Pipeline `[CLOSED]`

**What**: Transform uploaded vector files into queryable map layers
**Why**: Enables data publishers to share vector data via standard APIs
...
- [x] Supports standard filters (bbox, property, CQL)
...
**Acceptance Criteria**:
- [x] Implements OGC API - Tiles standard (MVT format)
- [x] Compatible with MapLibre, Leaflet, OpenLayers

---

## FEATURE 2: Raster Data Pipeline `[CLOSED]`

**What**: Transform uploaded GeoTIFFs into cloud-optimized format for API access
**Why**: Enables raster data hosting and serving via modern tile APIs
...
- [x] Dynamic tile serving enabled immediately
...
#### US 3.1: API Contract Documentation `[ACTIVE]`
**As a** DDH developer
...
|------|-------------|--------|
| T3.1.1 | Generate OpenAPI spec | Done |
| T3.1.2 | Deploy Swagger UI | Done |
| T3.1.3 | Review with DDH team | In Progress |

---

#### US 3.2: Identity & Access Configuration `[CLOSED]`
...
**As a** data scientist
**I want** direct data access to COG pixel values
**So that** I can extract data for analysis without downloading files

**Acceptance Criteria**:
- [x] Point queries (/cog/point/{lon}/{lat})
- [x] Statistics endpoint (/cog/statistics)
- [x] HTTP range requests supported for partial reads

---

#### US 5.3: Vector Tiles & OGC Features `[CLOSED]`
**As a** web developer
**I want** vector tiles and GeoJSON from PostGIS
**So that** I can render vector data efficiently

**Acceptance Criteria**:
- [x] Implements OGC API - Features and OGC API - Tiles
- [x] Works with any OGC-compatible mapping library

---

#### US 5.4: Multidimensional Data `[CLOSED]`
...
#### US 5.5: pgSTAC Mosaic Searches `[CLOSED]`
**As a** data analyst
...
- [x] Compatible with STAC Browser, pystac, and other standard clients

---

#### US 5.7: Service Operations `[CLOSED]`
...
**Acceptance Criteria**:
- [x] h3.cells table with geometries (res 2-7)
...
- [x] Cascade generation and country intersection
...
**Acceptance Criteria**:
- [x] Denormalized export to geo schema
...
- [x] Queue status and database statistics
...
- [x] Task viewer with logs and timeline
...
**Acceptance Criteria**:
- [x] Collection/item listing with thumbnails
- [x] Raster curator with map preview and approve/reject
...
- [x] Vector curator with MapLibre preview
