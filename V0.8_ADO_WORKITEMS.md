...
Structure:
  ...
  └── Feature (business capability)
        ├── User Story (deliverable) - delivers value to named persona
        ├── Enabler (foundation) - technical work enabling future capabilities
        └── Spike (exploration) - research/proof-of-concept
...
**Journey**: Data Publisher (B2B) → Governance Approval (B2B) → End User Access (B2C - Internal/Public)
...
- PUBLIC data → Internal service layer + External service layer (Cloudflare CDN → public)
...
**Flow**:
1. Platform admin grants B2B app Managed Identity access (F7: US 7.2) ✓
...
**Features**: F7 (Integration) → F2/F3 (Pipelines) → F5 (Service APIs) + F6 (Developer Portal)  
**Status**: ⚠ In Progress - Blocked by eService RITMs (see below)  
**Dependencies**: 
...
**Business Value**: Eliminates NetCDF → Zarr conversion work, saves TB-scale storage duplication, enables immediate access to existing NetCDF collections
...
1. ✓ Identity & Access (US 7.2) - COMPLETE
2. 🔄 API Documentation (US 7.1) - Review with ITSDA team pending
3. ⚠ Docker Deployment (RITM00009447599) - ACN approval required
4. ⚠ ADF External Delivery (RITM00009453692) - Contributor access required
5. ⚠ Entra Auth Migration (RITM00009453723) - MI authentication required
...
**Business Value**: Eliminates planned NetCDF → Zarr conversion work, saves TB-scale storage costs
...
5. **Tier 3 (B2C)**: OUO → Internal only, PUBLIC → Internal + External (Cloudflare)
...
- **External Service Layer**: PUBLIC-approved data only - Cloudflare CDN → public
...
**What**: Enable DDH to submit ETL jobs and consume resulting geospatial services
**Why**: DDH drives data publishing (B2B orchestration) and embeds data access for end users (B2C consumption)

**Integration Pattern**:
- **B2B (Orchestration)**: DDH submits vector/raster ETL requests via Platform API → Jobs execute → Data published
- **B2C (Consumption)**: DDH embeds OGC APIs (tiles, features, STAC) for end-user data access

**User Journey**:
1. **Tier 1 (Platform Ops)**: Configure DDH Managed Identity access (US 7.2) ✓
...
4. **Tier 2 (B2B - DDH ITSDA)**: Integrate Platform API for job submission (orchestration)
5. **Tier 2 (B2B - DDH ITSDA)**: Embed Service Layer APIs for data consumption
6. **Tier 3 (B2C - DDH users)**: Access geospatial features within DDH interface

**Consumed By**: DDH application (primary B2B client)
**Depends On**: Features 1, 2, 3, 4, 5 (entire platform stack)
...
**Value Stream Status**:
- ✓ **VS1: Vector Publishing** - Complete end-to-end
- ✓ **VS2: Raster Publishing** - Complete end-to-end
- ⚠ **VS3: B2B Integration** - In Progress (blocked by eService RITMs)
- 🔄 **VS4: VirtualiZarr NetCDF** - Planned (critical deliverable for DEC)
