...
1. **`GeospatialAsset` (Aggregate Root)** - The central entity representing a dataset managed through the Platform API. As the **Aggregate Root** (DDD pattern), it controls access to related entities and enforces all business invariants.
2. **Request/Job Separation** - Decoupling work orders from execution records
3. **Orthogonal State Machine** - Four independent state dimensions (Revision, Approval, Clearance, Processing)
...
- All access to jobs, requests, and revisions goes through the asset
- Enforces business invariants (e.g., can't approve if not pending_review)
- Single source of truth for asset state
- External systems reference via `request_id`, internally everything centers on `asset_id`

```
GeospatialAsset (Aggregate Root)
    │
    ├──◄ Job (N) ────────── job.asset_id → asset
    ├──◄ ApiRequest (N) ─── request.asset_id → asset
    └──◄ AssetRevision (N) ─ revision.asset_id → asset
```
