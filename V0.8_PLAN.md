...
## 18. DISK-BASED COG PROCESSING ✅ COMPLETE (25 JAN 2026)

**Epic**: E7.F7.20 - Large Raster Processing
**Status**: COMPLETE - 1.3GB WorldView-3 8-band files processing successfully

### Problem Statement

Docker worker was experiencing OOM kills when processing large rasters (>500MB):
- Memory watchdog only tracked Python RSS, missing GDAL C allocations
- "Disk-based" mode still loaded files into MemoryFile (RAM)
- Lock renewal had no logging, making timeout diagnosis difficult
- Container would be killed by Azure after ~10 minutes at 100% memory

### Solution Implemented

#### Phase 18.1: Memory Monitoring Fixes ✅

**File**: `core/docker_context.py`

1. **Cgroups-based memory tracking**: Read `/sys/fs/cgroup/memory.current` (v2) or `/sys/fs/cgroup/memory/memory.usage_in_bytes` (v1) instead of Python RSS
2. **INFO-level logging**: Changed memory logs from DEBUG to INFO for visibility
3. **On-demand checkpoints**: Added `log_memory_status()` method
...
#### Phase 18.2: Lock Renewal Logging ✅

**File**: `docker_service.py`

Added failure callback to `AutoLockRenewer`:
```python
def on_lock_renew_failure(renewable, error):
    logger.error(f"🔒❌ LOCK RENEWAL FAILED: {error}")
...
#### Phase 18.3: True Disk-Based Processing ✅

**Files**:
- `core/models/blob_mount_transfer.py` (NEW)
- `infrastructure/blob.py`
- `services/raster_cog.py`
- `services/handler_process_raster_complete.py`

**New Pydantic Models**:
- `BlobToMountRequest` - Request to stream blob → mounted filesystem
- `MountToBlobRequest` - Request to stream mount → blob
- `BlobMountTransferResult` - Transfer metrics

**New BlobRepository Methods**:
- `stream_blob_to_mount()` - Downloads blob in 32MB chunks to `/mounts/etl-temp`
- `stream_mount_to_blob()` - Uploads from mount to blob in 32MB chunks

**Processing Flow** (when `USE_ETL_MOUNT=true`):
```
STEP A: stream_blob_to_mount() → /mounts/etl-temp/input_{task_id}.tif
STEP B: Read metadata with rasterio
STEP C: cog_translate(file, file) → /mounts/etl-temp/output_{task_id}.cog.tif
STEP D: Read output metadata
STEP E: stream_mount_to_blob() → silver-cogs container
...
| Bug | Error | Fix |
|-----|-------|-----|
| Decorator arg order | `got multiple values for argument 'mount_path'` | Changed `stream_mount_to_blob(container, blob_path, mount_path)` - container first for `@dec_validate_container` |
| Missing task_id | `No such file or directory: input_unknown.tif` | Added `'_task_id': task_id` to `cog_params` dict |

### Results

**Test File**: WorldView-3 8-band multispectral (1.3GB input → 1.7GB COG)

| Metric | Before | After |
|--------|--------|-------|
| Memory Usage | OOM @ 100% | Stable 47% (3.9GB/8.3GB) |
| Python RSS | N/A (killed) | 0.75GB |
| Processing Time | Failed | 6.4 minutes |
| Success Rate | 0% | 100% |

### Commits

- `d5956f8`: Fix stream_mount_to_blob argument order for decorator compatibility
- `89fcf43`: Pass _task_id to create_cog for disk-based temp file naming

### Configuration

```bash
# Enable disk-based processing (Docker worker)
USE_ETL_MOUNT=true
ETL_MOUNT_PATH=/mounts/etl-temp
```

---

## 19. JOB VERSION TRACKING (F7.22)

### Overview

**Problem**: App version (`__version__`) is not captured in job execution metadata. When debugging issues or auditing historical jobs, there's no way to know which app version processed the job.

**Solution**: Capture version at job creation and preserve version history on resubmit.

### Current State

| Aspect | Status |
|--------|--------|
| `__version__` defined | ✅ `config/__init__.py:49` |
| Version in job metadata | ❌ Not captured |
| Metadata at creation | Limited: `{'description': ..., 'created_by': ...}` |
| Metadata at resubmit | Overwrites all: `{'resubmitted': True, ...}` |
| Metadata in API response | ❌ Not exposed |

### Design

#### Phase 1: Capture Version at Job Creation

**File**: `jobs/mixins.py` (lines 628-631)
...
metadata={
    ...
    'created_app_version': __version__  # Immutable original
}
```

#### Phase 2: Preserve History on Resubmit

**File**: `triggers/jobs/resubmit.py` (lines 390-401)
...
# Get original job's metadata before deletion
original_job = self.job_repo.get_job(job_id)
original_metadata = original_job.metadata if original_job else {}
original_created_at = original_job.created_at.isoformat() if original_job else None

# Build version history
existing_history = original_metadata.get('version_history', [])
if not existing_history and original_metadata.get('app_version'):
    # First resubmit - create history from original
    existing_history = [{
        'version': original_metadata.get('app_version'),
        'action': 'original_submission',
        'timestamp': original_created_at
    }]

# Add current resubmit to history
...
metadata={
    ...
    'resubmit_timestamp': str(uuid.uuid4())[:8],
    'app_version': __version__,
    'created_app_version': original_metadata.get('created_app_version', original_metadata.get('app_version')),
    'version_history': existing_history
}
```

#### Phase 3: Expose Metadata in API

**File**: `triggers/get_job_status.py` (after line 127)

```python
# Add metadata to response
if job_record.metadata:
    response["metadata"] = job_record.metadata
```

### Expected Output

**Initial submission (v0.7.33.4)**:
```json
{
  "jobId": "abc123...",
  "metadata": {
    "description": "Ingest raster to COG",
    "created_by": "RasterIngestJob",
    "app_version": "0.7.33.4",
    "created_app_version": "0.7.33.4"
  }
}
```

**After resubmit (v0.8.0)**:
```json
{
  "jobId": "abc123...",
  "metadata": {
    "resubmitted": true,
    "resubmit_timestamp": "a1b2c3d4",
    "app_version": "0.8.0",
    "created_app_version": "0.7.33.4",
    "version_history": [
      {"version": "0.7.33.4", "action": "original_submission", "timestamp": "2026-01-20T10:00:00Z"},
      {"version": "0.8.0", "action": "resubmit", "timestamp": "2026-01-26T14:30:00Z"}
    ]
  }
}
```
...
| File | Change |
|------|--------|
| `jobs/mixins.py` | Add `app_version` and `created_app_version` to metadata |
| `triggers/jobs/resubmit.py` | Preserve original version, build version history |
| `triggers/get_job_status.py` | Expose metadata field in API response |

### Testing Checklist

- [ ] Submit new job, verify `app_version` in metadata
- [ ] Resubmit job, verify `created_app_version` preserved
- [ ] Resubmit job multiple times, verify `version_history` accumulates
- [ ] GET `/api/jobs/status/{id}` returns metadata field
- [ ] Verify existing jobs without version don't break

### Implementation Notes

1. **Backward Compatibility**: Existing jobs have no version metadata. Code must handle `None` gracefully.
2. **Resubmit captures before delete**: Must read original job metadata BEFORE deleting the job record.
3. **Version history accumulates**: Multiple resubmits should append to history, not replace.
