...
**Version**: 0.8.4.2
**Status**: IN PROGRESS - dry_run Implementation Complete, Ready for Testing
...
1. **Bug Tracking** - Issues found during V0.8 testing (from V0.8_TESTING_PLAN.md)
...
4. **Release Control Validation** - 44-test plan for V0.8.4.1 lineage/versioning
5. **dry_run & Validation Tests** - 14-test plan for V0.8.4.2 pre-flight validation (NEW)
...
4. **Mount Behavior**:
   ...
   - Mount missing → Degraded state (warning, not failure), limited processing
...
|----|----------|-----------|-------------|--------|
| BUG-001 | MEDIUM | CoreMachine | `job_events` table fails - `null value in column "event_id"` | **FIXED** ✅ |
| BUG-002 | ~~MEDIUM~~ | Vector ETL | ~~KML parsing returns empty~~ NOT REPRODUCIBLE | **CLOSED** |
| BUG-003 | **HIGH** | Raster ETL | Disk-based COG skips CRS reprojection | **FIXED** ✅ |
| BUG-004 | MEDIUM | Raster ETL | MosaicJSON dead code causing errors | **FIXED** ✅ |
| BUG-005 | **HIGH** | Raster ETL | STAC collection result always empty for tiled jobs | **FIXED** ✅ |
| BUG-006 | ~~HIGH~~ | Platform API | ~~Approval API returns 404~~ NOT A BUG | **CLOSED** |
| BUG-007 | MEDIUM | Raster ETL | COGCreationData validation error in mount workflow | **FIXED** ✅ |
| BUG-008 | **HIGH** | Raster ETL | Tiled workflow STAC blob name mismatch | **FIXED** ✅ |
| BUG-009 | **HIGH** | Vector ETL | `overwrite=true` does NOT truncate table | **FIXED** ✅ |
| BUG-010 | **HIGH** | UI/Raster | Map button doesn't open collection mosaic | **FIXED** ✅ |
| BUG-011 | **HIGH** | STAC/TiTiler | Preview URLs fail for 3+ band imagery | **FIXED** ✅ |
| BUG-012 | **HIGH** | Platform/ETL | Overwrite race condition - async unpublish deletes new STAC | **FIXED** ✅ |
| BUG-013 | **HIGH** | Platform/Catalog | Catalog lookup returns `found: false` despite STAC item existing | **FIXED** ✅ |
| BUG-014 | **HIGH** | Raster ETL | Overwrite "no_change" path returns empty STAC dict | **FIXED** ✅ |
| BUG-015 | **HIGH** | Vector ETL | Vector jobs return `stac_item_id: null` - structure mismatch | **FIXED** ✅ |
| BUG-016 | **CRITICAL** | Approval API | Approve endpoint doesn't update classification or GeospatialAsset | **FIXED** ✅ |
...
#### BUG-005: STAC collection result always empty ✅ FIXED
...
#### BUG-009: Overwrite doesn't truncate table ✅ FIXED
...
#### BUG-012: Overwrite race condition ✅ FIXED
...
#### BUG-013: Catalog lookup fails despite STAC existing ✅ FIXED
...
#### BUG-014: Overwrite "no_change" path returns empty STAC dict ✅ FIXED
**Discovered**: 30 JAN 2026
...
#### BUG-015: Vector jobs return stac_item_id: null ✅ FIXED
...
#### BUG-016: Approve endpoint doesn't update classification or GeospatialAsset ✅ FIXED
**Discovered**: 31 JAN 2026
**Severity**: CRITICAL
...
| D-03 | Clearance upgrade (ouo→public) | clearance_state=public, made_public_at set |
| D-04 | Clearance downgrade (public→ouo) | clearance_state=ouo, warning in response |
...
- [ ] Run DRY-A-01 through DRY-A-02 (First Version)
- [ ] Run DRY-B-01 through DRY-B-04 (Existing Lineage)
- [ ] Run DRY-C-01 through DRY-C-04 (Submit Enforcement)
- [ ] Run DRY-D-01 through DRY-D-04 (Validate Endpoint Parity)

---

## Success Criteria

- [ ] All bugs from Part 1 verified FIXED
- [ ] All 86 Platform API tests pass (Part 2)
- [ ] Docker worker handles mount degradation gracefully (Part 3)
- [ ] Schema verification complete
- [ ] No orphaned records
- [ ] All 44 Release Control tests pass (Part 4)
- [ ] All 14 dry_run & Validation tests pass (Part 5)
- [ ] **Total**: 144 tests passing (86 + 44 + 14)
...
- [x] All 4 platform versioning columns exist with correct types ✅
- [x] All 5 lineage columns exist on `geospatial_assets` ✅
- [x] All 4 lineage indexes created ✅
- [x] DDH platform seeded with `uses_versioning=true`, `version_ref='version_id'` ✅
...
| RC-B-02 | Same nominal refs → same lineage ID | Generate for v1.0, v2.0 with same dataset/resource | Identical lineage_id |
| RC-B-03 | Different resource → different lineage ID | Change `resource_id` to "surabaya" | Different lineage_id |
| RC-B-04 | Order-independent generation | Swap order of nominal_refs list | Same lineage_id |
| RC-B-05 | Null nominal_refs → falls back to asset_id | `nominal_refs=None` | Uses full platform_refs hash |
| RC-B-06 | Empty platform uses fallback | Non-versioning platform | lineage_id = asset_id |

#### Acceptance Criteria - Lineage ID
- [ ] Lineage ID is deterministic (same input → same output)
...
|----|------|--------|----------|--------|
| RC-C-01 | Get lineage state for new lineage | `asset_service.get_lineage_state(platform_refs)` | `lineage_exists=False`, `suggested_ordinal=1` | ✅ PASS |
| RC-C-02 | Get lineage state for existing | After v1 created, call with v2 refs | `lineage_exists=True`, `current_latest` populated | ✅ PASS |
| RC-C-03 | Get latest in lineage | `asset_service.get_latest_in_lineage(lineage_id)` | Returns v1 asset | ✅ PASS |
| RC-C-04 | Get by version | `asset_service.get_asset_by_version(lineage_id, "v1.0")` | Returns correct asset | 🔄 TODO |
| RC-C-05 | Get version history | `asset_service.get_version_history(lineage_id)` | List ordered by ordinal DESC | 🔄 TODO |
| RC-C-06 | Create second version | Submit v2.0 with ordinal=2 | New asset, v1 `is_latest=False` | ✅ PASS |
| RC-C-07 | Flip is_latest on new version | After v2 created | Only v2 has `is_latest=True` | ✅ PASS |
| RC-C-08 | Version history includes both | Get history after v2 | Returns [v2, v1] | ✅ PASS |
| RC-C-09 | Retire version | Set v1 `is_served=False` | v1 not in served query | 🔄 TODO |
| RC-C-10 | Re-serve version | Set v1 `is_served=True` | v1 back in served query | 🔄 TODO |

#### Acceptance Criteria - Asset Service
- [x] `get_lineage_state()` correctly identifies new vs existing lineages ✅
- [x] `get_latest_in_lineage()` returns correct asset ✅
- [x] `get_version_history()` returns all versions ordered correctly ✅ (verified via DB query)
- [x] `is_latest` flag flips correctly on new version ✅
...
|----|------|----------|----------|--------|
| RC-D-01 | First version sets ordinal=1 | Submit v1.0 | `version_ordinal=1`, `is_latest=True` | ✅ PASS |
| RC-D-02 | Second version sets ordinal=2 | Submit v2.0 after v1 | `version_ordinal=2`, `is_latest=True` | ✅ PASS |
| RC-D-03 | Previous version de-latest | After v2 submit | v1 has `is_latest=False` | ✅ PASS |
| RC-D-04 | previous_asset_id links correctly | v2 created | v2.previous_asset_id = v1.asset_id | ✅ PASS |
| RC-D-05 | Overwrite same version | Submit v1.0 with overwrite=true | Same asset_id, revision incremented | ✅ PASS |
| RC-D-06 | Overwrite resets approval state | Overwrite approved asset | `approval_state=pending_review` | ✅ PASS |
| RC-D-07 | Skip version ordinal | Submit v3 when v2 expected | Validation error OR warning | ⚪ N/A - Auto-calculated |
| RC-D-08 | Concurrent version submit | Two v2 submits simultaneously | First succeeds, second fails constraint | ✅ PASS - DB constraint prevents corruption |

#### Test Execution Results (30 JAN 2026)

**Test Dataset**: `RC_LINEAGE_TEST/jakarta`

Submitted v1.0 → v2.0 → v3.0 via `/api/platform/submit`:

```
┌──────────┬─────────┬─────┬────────┬──────────┐
│  asset   │ version │ ord │ latest │   prev   │
├──────────┼─────────┼─────┼────────┼──────────┤
│ 64110673 │ v1.0    │   1 │ f      │ NULL     │
│ a27e20cb │ v2.0    │   2 │ f      │ 64110673 │
│ 5d6813a2 │ v3.0    │   3 │ t      │ a27e20cb │
└──────────┴─────────┴─────┴────────┴──────────┘
```
...
- `previous_asset_id` creates correct chain: v1 ← v2 ← v3
...
- `revision` increments on each overwrite (1→2→3)
- `approval_state` resets from `approved` → `pending_review`
- `clearance_state` resets from `ouo` → `uncleared`
...
- v1.0 job: `2bafeccfc71ad6cf...` → completed
- v2.0 job: `9878c0639d63e462...` → completed (18:02:47)
- v3.0 job: `cb7398624955a5e1...` → completed (18:02:52)
...
│  asset   │ version │ ord │ latest │   prev   │
├──────────┼─────────┼─────┼────────┼──────────┤
│ 5ee5b26f │ v1.0    │   1 │ f      │ NULL     │ ← Correct
│ ac910c7a │ v2.0    │   2 │ t      │ 5ee5b26f │ ← WRONG: both have ordinal 2
│ d2e086fc │ v3.0    │   2 │ t      │ 5ee5b26f │ ← WRONG: should be ordinal 3, is_latest conflict
└──────────┴─────────┴─────┴────────┴──────────┘
```
...
- [x] Version ordinals increment correctly ✅
- [x] `is_latest` flag always has exactly one TRUE per lineage ✅ (DB constraint enforced)
- [x] `previous_asset_id` creates correct chain ✅
- [x] Overwrite behavior works within same version ✅
- [x] Out-of-order ordinals handled appropriately ✅ (N/A - auto-calculated)
- [x] Concurrent submit prevented by DB constraint ✅ (31 JAN 2026)
...
- [x] Jobs table has `asset_id` and `platform_id` columns ✅
- [x] API Requests table has `asset_id` and `platform_id` columns ✅
- [x] All 4 linkage indexes exist ✅
- [x] Nullable columns (jobs may not have assets) ✅
...
- [x] **Schema**: All 12 schema tests pass (RC-A-01 to RC-A-12) ✅ Verified 30 JAN 2026
- [ ] **Lineage ID**: All 6 generation tests pass (RC-B-01 to RC-B-06) - Unit tests pending
- [x] **Asset Service**: 6/10 service method tests pass (RC-C-01 to RC-C-10) ✅ Core methods verified
- [x] **Version Management**: 7/8 version tests pass (RC-D-01 to RC-D-08) ✅ DB constraint prevents race condition
- [x] **Linkage**: All 8 linkage tests pass (RC-E-01 to RC-E-08) ✅ Verified 30 JAN 2026
- [x] **Total**: 33/44 tests passing (75%) ✅
...
## Part 5: dry_run & Validation Endpoint Tests (V0.8.4.2)
...
- **Workflow A**: `POST /api/platform/validate` → `POST /api/platform/submit`
- **Workflow B**: `POST /api/platform/submit?dry_run=true` → `POST /api/platform/submit`
...
- [ ] **DRY-A**: Both endpoints return `valid: true` for new lineage with no previous_version_id
- [ ] **DRY-B**: Correct validation for all previous_version_id scenarios
- [ ] **DRY-C**: Submit enforces validation (400 on invalid, 202 on valid)
- [ ] **DRY-D**: /validate and ?dry_run=true return identical responses
