...
**Created**: 16 FEB 2026
**Updated**: 16 FEB 2026 — Phase 2 rewritten for unified metadata architecture
...
### P1.1 — Add APP_PREFIX and Extension URL Constants
...
### P1.2 — Rewrite PlatformProperties (platform:* → ddh:*)
...
### P1.3 — Add ProvenanceProperties (new geoetl:* namespace)
...
### P1.6 — Delete STACItemProperties Composite
...
### P1.7 — Keep GeoProperties, PostGISProperties, STACItemCore Unchanged
...
### P1.9 — Update File Header and Docstring
...
### P3.1 — Accept Plain Dicts in insert_item()
...
### P3.2 — Accept Plain Dicts in insert_collection()
...
### P3.3 — Update B2B Queries (platform:* → ddh:*)
...
|-------|-------|-------|------------|
| **1: Models** | P1.1–P1.9 | `core/models/stac.py` only | Namespace alignment |
| **2: Unified Metadata → STAC** | P2.1–P2.7 | unified_metadata, service_stac_metadata, stac_metadata_helper, handler_process_raster_complete, new stac_renders | Canonical builder via `RasterMetadata.to_stac_item()`, always-on statistics, full cog_metadata |
| **3: Infrastructure** | P3.1–P3.3 | `infrastructure/pgstac_repository.py` only | Plain dicts + ddh:* queries |
| **4: Consumers** | P4.1–P4.14 | ~15 consumer files + grep sweep + re-ingest | Wire everything together |
