...
## V0.8.17.2 — Raster Render Configs + Expanded Types (12 FEB 2026)

### Summary
Raster render configurations are now persisted to PostgreSQL as source of truth (`app.raster_render_configs` and `app.cog_metadata`) before STAC metadata is generated. The `RasterType` enum is expanded from 7 to 14 values, and users can now specify a `default_ramp` colormap at job submission time.

### Changes

**Expanded RasterType Enum** (14 values):
- **Physical types** (auto-detectable): `rgb`, `rgba`, `dem`, `categorical`, `multispectral`, `nir`, `continuous` (new), `vegetation_index` (new)
- **Domain types** (user-specified): `flood_depth`, `flood_probability`, `hydrology`, `temporal`, `population` (all new)

**New ColorRamp Enum** (28 curated TiTiler colormaps):
- Organized by use case: sequential, terrain, water, heat, vegetation, classification, specialized
- Users submit `default_ramp` parameter to override type-based colormap defaults

**Hierarchical Type Validation**:
- Domain types refine physical detections using compatibility groups
- `flood_depth` + detected `dem` → accepted (uses `blues` colormap instead of `terrain`)
- Genuine mismatches (e.g., `dem` for 3-band RGB) still fail

**Improved Single-Band Detection**:
- Vegetation index auto-detected: single-band float in [-1, 1] range
- DEM detection: smooth gradient analysis (unchanged)
- New `continuous` catch-all replaces DEM as the default for single-band data

**Source of Truth Pipeline** (Phase 3 → Phase 4):
- Phase 3a: `app.cog_metadata` (physical COG properties)
- Phase 3b: `app.raster_render_configs` (visualization parameters)
- Phase 4: STAC properties derived from Phase 3 data

### Files Modified
- `core/models/enums.py` — RasterType + ColorRamp enums
- `core/models/raster_render_config.py` — Extended colormap selection + `default_ramp`
- `services/raster_validation.py` — Hierarchical validation + improved detection
- `services/handler_process_raster_complete.py` — Wire `default_ramp` to render config
- `services/stac_metadata_helper.py` — Updated colormap fallback logic
- `jobs/process_raster_docker.py` — Added `default_ramp` parameter
