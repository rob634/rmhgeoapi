...
4. Use Azure CLI to list container contents:
   ```bash
   az storage blob list --container-name <bronze-storage> --account-name <geo-storage> --output table
   ```
...
2. Create container if needed:
   ```bash
   az storage container create --name my-container --account-name <geo-storage>
   ```
3. Use standard container names: `<bronze-storage>`, `silver-cogs`, `silver-vectors`
...
curl https://<platform-api-url>/api/dbadmin/tasks/{JOB_ID}
```
