...
```bash
curl -X POST "https://<platform-api-url>/api/dbadmin/maintenance/full-rebuild?confirm=yes"
```
...
| Property | Value |
|----------|-------|
| **Server** | `<postgres-server>.postgres.database.azure.com` |
| **Database** | `geopgflex` |
...
-- Replace '<db-reader-user>' with your reader managed identity name
GRANT USAGE ON SCHEMA geo TO <db-reader-user>;
GRANT SELECT ON ALL TABLES IN SCHEMA geo TO <db-reader-user>;
ALTER DEFAULT PRIVILEGES IN SCHEMA geo GRANT SELECT ON TABLES TO <db-reader-user>;
```
...
export PGHOST=<postgres-server>.postgres.database.azure.com
export PGPORT=5432
...
env = os.environ.copy()
env.update({
    'PGHOST': '<postgres-server>.postgres.database.azure.com',
    'PGPORT': '5432',
...
-- This allows <db-reader-user> to query STAC collections and items
GRANT pgstac_read TO <db-reader-user>;
