...
curl https://<platform-function-app>-.../api/storage/containers

# Filter to bronze zone only
curl "https://<platform-function-app>-.../api/storage/containers?zone=bronze"

# Filter by container name prefix
curl "https://<platform-function-app>-.../api/storage/containers?prefix=silver-"
```
...
curl "https://<platform-function-app>-.../api/containers/<bronze-storage>/blobs?suffix=.tif&limit=10"

# List blob names only (lightweight)
curl "https://<platform-function-app>-.../api/containers/<bronze-storage>/blobs?metadata=false&limit=100"

# List from silver zone with prefix filter
curl "https://<platform-function-app>-.../api/containers/silver-cogs/blobs?zone=silver&prefix=maxar/"
```
...
```bash
# Basic inventory
curl -X POST "https://<platform-function-app>-.../api/jobs/submit/inventory_container_contents" \
  -H "Content-Type: application/json" \
  -d '{"container_name": "<bronze-storage>", "limit": 100}'

# Geospatial inventory with TIF filter
curl -X POST "https://<platform-function-app>-.../api/jobs/submit/inventory_container_contents" \
  -H "Content-Type: application/json" \
  -d '{"container_name": "<bronze-storage>", "analysis_mode": "geospatial", "suffix": ".tif"}'
```
