...
from core.models.workflow import WorkflowDefinition, TaskDefinition
...
class JobFactory:
    ...
    def create_controller(job_type: str) -> BaseController:
        ...
        NO FALLBACKS: Wrong job_type = loud failure.

        Args:
            job_type: The exact job type as registered (e.g. 'hello_world' or 'sb_hello_world')

        Returns:
            Instantiated controller with workflow attached

        Raises:
            ValueError: If job_type is not registered (with list of available types)
        ...
        # Get controller - exact match only, no prefixing, no fallbacks
        try:
            controller_class = JobFactory._catalog.get_controller(job_type)
            metadata = JobFactory._catalog.get_metadata(job_type)
        except (ValueError, KeyError) as e:
            # Loud failure with helpful error message
            available_types = JobFactory._catalog.list_job_types()
            raise ValueError(
                f"Job type '{job_type}' not registered. "
