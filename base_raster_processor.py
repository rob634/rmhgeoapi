...
Provides shared storage, configuration, and utility methods.
"""
...
class BaseRasterProcessor(ABC):
    ...
    - Shared utility methods for blob operations
    """
    ...
    def get_blob_url(self, container: str, blob_name: str) -> str:
        """
        Get SAS URL for blob access.
        Single implementation used by all subclasses.
    ...
    def check_file_exists(self, container: str, blob_name: str) -> Tuple[bool, Optional[int]]:
        ...
        Returns:
            Tuple of (exists: bool, size_bytes: int or None)
        """
        try:
            blob_client = self.storage.blob_service.get_blob_client(
                container=container,
                blob=blob_name
            )
    ...
    def handle_storage_error(self, error: Exception, operation: str) -> Dict[str, Any]:
        """
    ...
    def copy_blob(self, source_container: str, source_blob: str, 
        ...
        """
        try:
            source_client = self.storage.blob_service.get_blob_client(
                container=source_container,
                blob=source_blob
            )
            dest_client = self.storage.blob_service.get_blob_client(
