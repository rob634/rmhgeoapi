...
**Seq 4 (Unpublish)**: Created a fresh dataset (`sg-unpub-test`), submitted, approved, then unpublished successfully via `request_id` + `deleted_by` + `force_approved=true`. **Note**: As of v0.9.16.0+ fix, `reviewer` is now the standard field (consistent with approve/reject/revoke). `deleted_by` is still accepted but deprecated with a warning log.
...
**NOTE (SG14-3 — FIXED)**: At time of SIEGE run, status used `/vsiaz/`/`abfs://` while catalog used `https://` scheme. Post-fix, catalog URLs now use `/vsiaz/` (raster) and `abfs://` (zarr) — consistent with status endpoint.
...
|---|-----|----------|----------|-------------|--------------|
| 1 | SG14-1 | MEDIUM | STAC Lifecycle | Zarr STAC materialization happens at job completion, NOT at approval time. `stac_collection` and `stac_item` in the services block are populated immediately when processing completes, before any approve call. The SIEGE spec (and siege_config.json) expects these to be null pre-approval. Raster correctly has null pre-approval (Seq 1 verified). This means zarr items are visible in the STAC catalog before approval -- potential data governance concern. | Submit any .nc or .zarr file, poll until completed, check `services.stac_collection` -- it will be non-null. | **✅ FIXED** (08 MAR 2026): Gated STAC URL visibility in `_build_services_block()` behind `approval_state == 'approved'`. STAC URLs now null pre-approval for ALL data types (raster and zarr). TiTiler serves tiles directly via `/vsiaz/` and `abfs://` without STAC. |
| 2 | SG14-2 | MEDIUM | Revocation | Cannot revoke non-latest approved release when another release at higher ordinal exists and is revoked. In Seq 18: v1 and v2 both approved. Revoke v2 succeeds (200). Then revoke v1 fails with 400: "approval_state is 'revoked', expected 'approved'". Root cause: `_resolve_release()` path 3 (`get_by_request_id()`) returns highest ordinal regardless of approval_state. When v2 is revoked, it still resolves v2 instead of v1. | Submit 2 versions, approve both (v1, v2), revoke v2, then try to revoke v1. | **✅ FIXED** (08 MAR 2026): `_resolve_release()` path 3 now checks approval_state for revoke operations. If resolved release isn't approved, falls back to `get_latest()` which filters `approval_state='approved'`. |
| 3 | SG14-3 | LOW | URL Scheme | Status `services.service_url` uses `/vsiaz/` or `abfs://` schemes while catalog `titiler_urls`/`xarray_urls` use `https://rmhstorage123.blob.core.windows.net/`. Both resolve to the same data but consumers seeing different URLs from different endpoints may be confused. | Compare `services.service_url` from `/api/platform/status` with `tiles.tilejson` from `/api/platform/catalog/lookup`. | **✅ FIXED** (08 MAR 2026): Catalog URLs changed to use `/vsiaz/` (raster) and `abfs://` (zarr) — consistent with status endpoint. |
| 4 | SG14-4 | LOW | Config | `siege_config.json` specifies `data_type_override: "zarr"` and the user prompt says to use `data_type=zarr` in submit body, but `PlatformRequest` has `extra='forbid'` and `data_type` is NOT a field -- it is a computed property from file extension. Submitting with `data_type` in the body returns 400. The `.nc` and `.zarr` extensions auto-detect correctly without any override. | Submit with `"data_type": "zarr"` in body -- gets 400 "Extra inputs are not permitted". | Cosmetic — no fix needed. |
| 5 | SG14-5 | LOW | API Surface | Unpublish endpoint (`/api/platform/unpublish`) uses `deleted_by` instead of `reviewer`, and does not accept `reason`/`reviewer` fields. This diverges from the approve/reject/revoke parameter pattern. | POST to `/api/platform/unpublish` with `reviewer` field -- gets 400 "not valid parameter". | **✅ FIXED** (08 MAR 2026): `reviewer` is now the standard field (matches approve/reject/revoke). `deleted_by` still accepted but deprecated with warning log. |
...
**Post-SIEGE Fixes (08 MAR 2026)**:
- **SG14-1 FIXED**: STAC URLs in services block gated behind `approval_state == 'approved'` in `_build_services_block()`. Null pre-approval for all data types.
- **SG14-2 FIXED**: `_resolve_release()` path 3 in `trigger_approvals.py` now checks approval_state for revoke operations. Falls back to `get_latest()` (filters `approval_state='approved'`).
- **SG14-3 FIXED**: Catalog URLs in `platform_catalog_service.py` changed from `https://{account}.blob.core.windows.net/` to `/vsiaz/` (raster) and `abfs://` (zarr).
- **SG14-5 FIXED**: Unpublish endpoint now accepts `reviewer` as standard field (matches approve/reject/revoke). `deleted_by` still accepted but logs deprecation warning.
- SG14-4 LOW: No fix needed — cosmetic config issue.
