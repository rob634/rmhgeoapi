...
**Created**: 30 JAN 2026
**Updated**: 31 JAN 2026
**Status**: PHASE A & B COMPLETE, Architecture Decisions Finalized
...
|----------|------|---------|
| Implicit Lineage | 31 JAN 2026 | No formal `app.lineages` table. Lineage is an emergent grouping via `lineage_id` column on `GeospatialAsset`. See [Architecture Decision: Implicit Lineage](#architecture-decision-implicit-lineage). |
| `?version=latest` Resolution | 31 JAN 2026 | Use `is_latest` flag on assets. Service Layer (geotiler) handles routing, not ETL layer. |
...
2. **TIER 6**: Create `/api/platform/validate` endpoint (or use `?dry_run=true` - see [DRY_RUN_IMPLEMENTATION.md](docs_claude/DRY_RUN_IMPLEMENTATION.md))
3. **TIER 7**: Enhance `/api/platform/submit` with version params
4. **TIER 8**: STAC Collection ID generation
5. **TIER 9**: Service Layer integration (geotiler repo) - `?version=latest` routing is geotiler's responsibility
...
**Note**: This is geotiler's responsibility, not rmhgeoapi. See [Architecture Decision: Implicit Lineage](#architecture-decision-implicit-lineage) for why routing lives in the Service Layer.

- [ ] Add `?version=latest` resolution: Query `WHERE is_latest = TRUE`
- [ ] Add `?version=v1.0` resolution: Query `WHERE platform_refs->>'version_id' = ?`
- [ ] Add `/versions` list endpoint
- [ ] Respect `is_served` flag in queries
- [ ] Accept `skip`/`top` as pagination aliases
- [ ] Implement routing/proxy layer to translate version queries to TiTiler/TiPG URLs
...
3. **Formal Lineage Entity vs Implicit Lineage**: ✅ RESOLVED (31 JAN 2026)
   - **Decision**: Use implicit lineage (no `app.lineages` table)
   - See [Architecture Decision: Implicit Lineage](#architecture-decision-implicit-lineage) section below
...
## Architecture Decision: Implicit Lineage

**Decision Date**: 31 JAN 2026
**Status**: APPROVED
**Decision**: Use implicit lineage via `lineage_id` column on `GeospatialAsset`. No formal `app.lineages` table.

### Context

B2B apps (DDH) think in terms of "datasets" that have multiple versions. They want:
- `?version=latest` to always resolve to the newest version
- `?version=v2.0` to resolve to a specific version
- URLs like `/tiles/floods/jakarta?version=latest`

The question was: do we need a formal Lineage entity (separate table) to support this?

### Decision: Implicit Lineage

**Lineage is an emergent grouping**, not a first-class entity:

```
┌────────────────────────────────────────────────────────────────┐
│  Lineage Definition (Implicit)                                 │
│                                                                │
│  lineage_id = hash(platform_id + nominal_refs)                 │
│                                                                │
│  Where nominal_refs = refs WITHOUT version                     │
│  For DDH: ["dataset_id", "resource_id"]                        │
│                                                                │
│  A "lineage" = all GeospatialAssets with the same lineage_id   │
│  Lineage "exists" when COUNT(assets with lineage_id) > 0       │
└────────────────────────────────────────────────────────────────┘
```

### Why This Works for `?version=latest`

The `is_latest` flag on `GeospatialAsset` provides the mechanism:

```sql
-- ?version=latest
SELECT blob_path, stac_item_id
FROM app.geospatial_assets
WHERE lineage_id = 'abc123...'
  AND is_latest = TRUE
  AND is_served = TRUE
  AND deleted_at IS NULL;
...
**A formal Lineage table would not simplify these queries.**

### Where the Complexity Actually Lives

The `?version=latest` complexity is in the **Service Layer routing**, not the data model:

```
┌─────────────────────────────────────────────────────────────────┐
│  Request: /tiles/floods/jakarta?version=latest                  │
│                                                                 │
│  Service Layer (geotiler) must:                                 │
│  1. Parse "floods" + "jakarta" → compute lineage_id             │
│  2. Query: WHERE lineage_id = ? AND is_latest = TRUE            │
│  3. Get blob_path from result                                   │
│  4. Proxy/redirect to TiTiler with actual blob URL              │
└─────────────────────────────────────────────────────────────────┘
```

| Layer | Responsibility | Formal Lineage Needed? |
|-------|----------------|------------------------|
| **rmhgeoapi (ETL)** | Set `is_latest=TRUE` on new version, flip old to FALSE | ❌ No |
| **geotiler (Service)** | Route `?version=latest` → query → resolve → proxy | ❌ No |
| **Database** | Index on `(lineage_id, is_latest)` for fast lookup | ❌ No |

### Service Layer Implementation (geotiler)

The routing logic lives in the Service Layer, not rmhgeoapi:

```python
# geotiler routing (pseudocode)
@app.get("/tiles/{dataset}/{resource}")
async def tiles(dataset: str, resource: str, version: str = "latest"):
    lineage_id = compute_lineage_id("ddh", {
        "dataset_id": dataset,
    ...
    if version == "latest":
        asset = db.query(
            "SELECT * FROM geospatial_assets "
            "WHERE lineage_id = ? AND is_latest = TRUE AND is_served = TRUE",
            lineage_id
        )
    else:
        asset = db.query(
            "SELECT * FROM geospatial_assets "
            "WHERE lineage_id = ? AND platform_refs->>'version_id' = ? AND is_served = TRUE",
            lineage_id, version
        )
    ...
    # Proxy to TiTiler with resolved blob_path
    return proxy_to_titiler(asset.blob_path, z, x, y)
```

### When to Reconsider

A formal `app.lineages` table would be needed if:

| Requirement | Why Formal Lineage Helps |
|-------------|--------------------------|
| Lineage-level metadata | Store description, tags, owner that apply to ALL versions |
| Lineage-level access control | "This entire dataset is OUO" vs per-version control |
| Pre-registration | B2B wants to "claim" a lineage before submitting data |
| Lineage listing | "Show me all lineages for platform DDH" without scanning assets |

**Current assessment**: None of these are required for V0.8. The implicit approach is sufficient.

### Summary

| Aspect | Approach |
|--------|----------|
| Lineage storage | Implicit via `lineage_id` column on `GeospatialAsset` |
| Lineage existence | Derived: `COUNT(*) > 0` for that lineage_id |
| `?version=latest` | Query with `is_latest = TRUE` flag |
| `?version=v2.0` | Query with `platform_refs->>'version_id' = 'v2.0'` |
| Routing logic | Service Layer (geotiler) responsibility |
| Formal Lineage table | Not needed for V0.8 |
