...
**Date**: 21 FEB 2026 (assessment), 23 FEB 2026 (updated post-implementation)
**Version Assessed**: v0.8.24.0 (originally assessed at v0.8.20.1)
**Overall Rating**: 8.5/10 — Asset/Release model implemented, status endpoint restructured, full QA pass
**Architecture Decision**: Asset/Release entity split — IMPLEMENTED (v0.8.22.0–v0.8.24.0)
...
| Risk | Impact | Where | Status (23 FEB) |
|------|--------|-------|-----------------|
| **psycopg3 Enum adapter untested** | All asset updates could 500 if ApprovalState/ClearanceState Enums don't serialize | `infrastructure/postgresql.py` | **Mitigated** — QA tests exercised all enum transitions successfully |
| **STAC materialization is immutable** | Cached `cog_metadata.stac_item_json` has no re-derivation path; double-approval or corrupted JSON = silent failure | `services/asset_approval_service.py` | Open — stac_item_json now on Release, but re-derivation still pending |
| **3-tier asset fallback** added ad-hoc | asset_id sometimes missing from job record (QA found this 20 FEB); fallback behavior undocumented | `triggers/trigger_approvals.py` | **RESOLVED** — Asset/Release split eliminates the need; Release has direct `job_id` FK |
...
### What This Eliminates — ALL VERIFIED IN QA (23 FEB 2026)

| V0.8 Problem | V0.9 Resolution | QA Test |
|-------------|-----------------|---------|
| `assign_version()` mutates identity | Version lives on Release; Asset identity never changes | A5, A10 |
| Revoke-first workflow (409 block) | Draft release coexists with approved releases under same asset | A7, B4 |
| 3-tier asset_id fallback in approvals | Job FK points directly at `release_id` | C1 |
| STAC dict cached on wrong entity | `stac_item_json` on Release, not shared across versions | A5, A6 |
| 10+ mutation methods on one repository | Asset repo: create, soft-delete. Release repo: approve, reject, revoke, update_processing | Full suite |
| `AssetRevision` as proto-artifact | Replaced entirely by Release history | A12 |
| `lineage_id` as separate concept | Asset IS the lineage — `SELECT * FROM releases WHERE asset_id=X ORDER BY ordinal` | H2 |
| `revision` vs `version_ordinal` confusion | `revision` = overwrite counter (within release), `ordinal` = version sequence (across releases) | D3 |
| Draft → versioned identity collision | Different release_ids, no PK collision, no revoke-first needed | A7, B4 |
| `"draft"` in naming | Ordinal-based naming (`ord1`, `ord2`), finalized at approval | A3, A9, B2, B5 |
| Status endpoint 404 on asset_id/release_id | Auto-detect lookup across all ID types | H1-H4 |
...
|----------|--------|-------------------------------|
| **V0.9_ASSET_MODEL.md** | **IMPLEMENTED** (v0.8.22.0–v0.8.24.0) | Core spec — fully implemented and QA-verified |
| **V0.9_PGSTAC.md** | ACTIVE — independent parallel | Catalog layer; Phase 2+4 now unblocked by asset model completion |
| **V0.9_PGSTAC_TODO.md** | IN PROGRESS — Phase 1+3 complete, Phase 2+4 pending | Phase 4 consumer updates can now proceed without double-edit risk |
| **V0.9_TEST.md** | ACTIVE — 2 QA runs complete | Full test suite covering lifecycle, naming, status restructuring |
...
1. **Asset/Release schema** (V0.9_ASSET_MODEL.md) — **COMPLETE** (v0.8.22.0–v0.8.24.0)
2. **PGSTAC Phase 2-3** (V0.9_PGSTAC_TODO.md) — Now unblocked, can proceed
3. **PGSTAC Phase 4** (consumer updates) — Now unblocked; asset model triggers are stable
