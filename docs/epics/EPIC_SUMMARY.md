# Epic & Feature Summary

**Updated**: 12 JAN 2026 | ✅ Complete | 🚧 In Progress | 📋 Planned | 🔵 Backlog

---

## E12: Interfaces
- F12.1 ✅ Interface Cleanup
- F12.2 ✅ HTMX Integration
- F12.3 ✅ Interface Migration
- F12.EN1 📋 Helper Enhancements
- F12.4 📋 System Dashboard
- F12.5 📋 Pipeline Workflow Hub
- F12.6 📋 STAC & Raster Browser
- F12.7 📋 OGC Features Browser
- F12.8 📋 API Documentation Hub

## E9: Large Data
- F9.1 ✅ FATHOM Flood ETL
- F9.2 📋 FATHOM Global Expansion
- F9.3 📋 CMIP6 Climate Data
- F9.4 📋 VirtualiZarr Integration
- F9.5 📋 Kerchunk Reference Files
- F9.6 🚧 TiTiler Services
- F9.7 📋 Zarr Query API
- F9.8 📋 Time Series Extraction
- F9.9 📋 Multi-Model Ensembles
- F9.10 📋 Data Subsetting

## E8: GeoAnalytics
- F8.1 ✅ H3 Grid Infrastructure
- F8.2 ✅ H3 Source Catalog
- F8.3 ✅ Raster Zonal Aggregation
- F8.4 📋 Vector Aggregation
- F8.5 📋 Temporal Aggregation
- F8.6 📋 Multi-Source Composites
- F8.7 📋 Building Flood Exposure
- F8.8 ✅ H3 Statistics Query API
- F8.9 ✅ H3 OGC Features Export
- F8.10 📋 Data Browser UI
- F8.11 📋 H3 Visualization
- F8.12 📋 Export Interface
- F8.13 📋 Cross-Resolution Analytics
- F8.14 📋 Performance Optimization

## E7: Pipeline Infrastructure
- F7.1 ✅ Pipeline Infrastructure
- F7.2 🟡 IBAT Reference Data
- F7.3 ✅ Collection Ingestion Pipeline
- F7.4 ✅ Pipeline Observability
- F7.5 📋 Pipeline Builder UI
- F7.6 📋 ACLED Conflict Data
- F7.7 📋 Static Reference Data
- F7.8 ✅ Unified Metadata Architecture (VectorMetadata)
- F7.9 🚧 RasterMetadata Architecture
- F7.10 ✅ Metadata Consistency Enforcement
- F7.11 🚧 STAC Catalog Self-Healing
- F7.12 ✅ Docker Worker Infrastructure
- F7.13 🚧 Docker Job Definitions (Phase 1 complete)
- F7.14 🔵 Dynamic Task Routing (optional)
- F7.15 📋 HTTP-Triggered Docker Worker
- F7.16 ✅ Code Maintenance (db_maintenance split)
- F7.17 ✅ Job Resubmit & Platform Features

## E4: Externalization 🔥 PRIORITY
- F4.1 📋 Publishing Workflow
- F4.2 🚧 ADF Python Integration
- F4.3 🚧 External Delivery Infrastructure
- F4.4 📋 ADF Infrastructure & Pipelines
- F4.5 🔵 Database-to-Database Pipelines

## E3: DDH Coordination
- F3.1 ✅ API Contract Documentation
- F3.2 📋 Identity & Access Configuration
- F3.3 📋 Environment Provisioning
- F3.4 📋 Integration Verification
- F3.5 🔵 Job Completion Callbacks
- F3.6 ✅ Health & Diagnostics
- F3.7 ✅ Error Telemetry
- F3.8 🔵 Verbose Validation

## E2: Raster Data ✅ Operational
- F2.1 ✅ Raster ETL Pipeline
- F2.2 ✅ TiTiler Integration
- F2.3 ✅ Raster STAC Integration
- F2.4 ✅ Raster Unpublish
- F2.5 ✅ Raster Data Extract API
- F2.6 ✅ Large Raster Support
- F2.7 📋 Raster Collection Pipeline
- F2.8 📋 Classification & Detection
- F2.9 ✅ STAC-Integrated Raster Viewer
- F2.10 🚧 Add Rasters to Existing Collections

## E1: Vector Data ✅ Operational
- F1.1 ✅ Vector ETL Pipeline
- F1.2 ✅ OGC Features API
- F1.3 ✅ Vector STAC Integration
- F1.4 ✅ Vector Promotion Workflow
- F1.5 ✅ Vector Map Viewer
- F1.6 ✅ Vector Unpublish
- F1.7 📋 OGC Styles Basic
- F1.8 📋 OGC Styles Advanced

---

## Totals

| Epic | ✅ | 🚧 | 📋 | 🔵 |
|------|:--:|:--:|:--:|:--:|
| E12 | 3 | 0 | 6 | 0 |
| E9 | 1 | 1 | 8 | 0 |
| E8 | 5 | 0 | 9 | 0 |
| E7 | 8 | 3 | 4 | 1 |
| E4 | 0 | 2 | 2 | 1 |
| E3 | 3 | 0 | 3 | 2 |
| E2 | 7 | 1 | 2 | 0 |
| E1 | 6 | 0 | 2 | 0 |
| **Total** | **33** | **7** | **36** | **4** |

---

**Details**: See `E{n}_*.md` | **Delivered**: See `COMPLETED_FEATURES.md`
