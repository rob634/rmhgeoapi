...
3. **Raster ETL**: Single unified handler with automatic tiling decision
   - File ≤ 2GB → Single COG output
   - File > 2GB → Tiled output (multiple COGs with individual STAC items)
4. **Mount Behavior**:
   - Mount present → Any file size, streaming I/O
   - Mount missing → Degraded state (warning, not failure), reject >1GB
...
| BUG-009 | **HIGH** | Vector ETL | `overwrite=true` does NOT truncate existing table. Resubmit with overwrite results in duplicate data (old + new rows). Table `acled_full_v8_testing_v10` has 5.1M rows (2x expected 2.57M) from two ETL batches. | **FIXED** ✅ | 27 JAN |
...
#### BUG-001: job_events event_id null constraint ✅ FIXED
**Error**:
```
...
#### BUG-003: Disk-based COG skips CRS reprojection ✅ FIXED
...
- EPSG:32750 (UTM 50S) → EPSG:4326 ✅
- EPSG:32648 (UTM 48N) → EPSG:4326 ✅
- EPSG:4326 (no reprojection) → EPSG:4326 ✅
**Discovered**: 26 JAN - During RAS-01 testing, job `7715b1f2...` output showed `proj:epsg: 32750` instead of 4326.

#### BUG-004: MosaicJSON dead code in tiled workflow ✅ FIXED
**Error**:
```
...
- ✅ V0.8 handlers (`handler_process_raster_complete.py`) - No MosaicJSON
- ⚠️ Deprecated handlers (`handler_process_large_raster_complete.py`) - Still has MosaicJSON phase but handler is deprecated (remove in V0.9)
- ⚠️ Legacy Function App jobs (`process_large_raster_v2.py`, `process_raster_collection_v2.py`) - Still have MosaicJSON stages (remove in V0.9)
...
#### BUG-005: STAC collection result always empty for tiled jobs ✅ FIXED
...
#### BUG-006: Approval API returns 404 ✅ CLOSED (Not a Bug)
...
#### BUG-007: COGCreationData validation error in mount-based workflow ✅ FIXED
**Error**:
```
...
#### BUG-008: Tiled workflow STAC fails - blob name mismatch ✅ FIXED
**Error**:
```
...
**Root Cause**: `raster_cog.py` adds tier suffix to blob name (e.g., `tile_0000.tif` → `tile_0000_analysis.tif`), but `handler_process_raster_complete.py` was returning the original name without suffix to the STAC phase.
...
#### BUG-009: Vector ETL overwrite=true does NOT truncate table ✅ FIXED
...
- Job `79c3e3eb...` with `overwrite=true` → table dropped and recreated (3,301 rows, NOT 6,602)
- Logs confirmed: `🗑️ Dropping existing table geo.new_hexagons_v8_testing_v10 (overwrite=true)`
**Discovered**: 27 JAN - Resubmit of acled_export.csv with overwrite=true.

#### BUG-012: Overwrite race condition - async unpublish deletes STAC after ETL creates it ✅ FIXED
...
3. ETL job completed first → table dropped, recreated, data loaded, STAC item created
4. Unpublish job ran AFTER ETL → deleted the STAC item that was just created
**Fix Applied** (v0.7.35.4 - 29 JAN 2026):
...
- STAC item preserved: `⏭️ STEP 2: Item postgis-geo-new_hexagons_v8_testing_v10 already exists in PgSTAC - skipping insertion (idempotent)`

**Verification - Raster Checksum**: Tested 29 JAN 2026
- **Initial job**: `0c81cd89...` (tiny-raster2, 150MB, overwrite=true) → full_reprocess (no prior checksum)
- **Overwrite job**: Same params with overwrite=true → `action: "no_change"` in 4.1 seconds
...
#### BUG-010: UI "Map" button doesn't open collection mosaic ✅ FIXED (by BUG-011)
...
#### BUG-011: Preview URLs fail for 3+ band imagery without bidx= ✅ FIXED
...
|------------|-------|--------------|---------------------------|
| giant-dsm-v8-testing-v10 | 1 (float32) | ✅ HTTP 200 | ✅ HTTP 200 |
| ras-05-v8-testing-v10 | 8 (uint16) | ❌ HTTP 500 | ✅ HTTP 200 |
...
|----------|-------|----------|-------------|
| [A.1 QA Critical Path](#a1-qa-critical-path-tests) | 24 | **P0 CRITICAL** | Submit → Status → Approve → Unpublish workflow |
| [A.2 Platform Diagnostics](#a2-platform-diagnostics-tests) | 4 | P1 HIGH | Health, failures, lineage, validate |
| [A.3 Platform Catalog](#a3-platform-catalog-tests) | 4 | P1 HIGH | STAC catalog B2B access |
| [A.4 Platform Approval Admin](#a4-platform-approval-admin-tests) | 12 | P1 HIGH | List, filter, revoke approvals |

**Part A Total**: 44 tests
...
The core DDH workflow: Submit → Poll Status → Approve → Unpublish
...
| QA-04 | Submit with overwrite=true | POST /api/platform/submit | Handler drops/replaces data directly, new job created | [x] ✅ 29 JAN - BUG-012 fixed race condition |
...
| ID | Test | Endpoint | Expected | Status |
|----|------|----------|----------|--------|
| QA-13 | Approve by request_id | POST /api/platform/approve | Returns success, status=approved | [ ] |
| QA-14 | Approve by job_id | POST /api/platform/approve | Returns success, status=approved | [ ] |
| QA-15 | Approve already approved | POST /api/platform/approve (same) | Idempotent success or appropriate message | [ ] |
| QA-16 | Approve invalid ID | POST /api/platform/approve (bad ID) | Returns error with helpful message | [ ] |

### A.1.4 Unpublish Endpoint

| ID | Test | Endpoint | Expected | Status |
|----|------|----------|----------|--------|
| QA-17 | Unpublish by request_id | POST /api/platform/unpublish | Returns success, monitor_url | [ ] |
| QA-18 | Unpublish by job_id | POST /api/platform/unpublish | Returns success, monitor_url | [ ] |
| QA-19 | Unpublish by DDH IDs | POST /api/platform/unpublish (dataset/resource/version) | Auto-detects, unpublishes | [ ] |
| QA-20 | Unpublish dry_run | POST /api/platform/unpublish (dry_run: true) | Returns what would be deleted, no action | [ ] |
| QA-21 | Unpublish non-existent | POST /api/platform/unpublish (invalid) | Returns appropriate error | [ ] |

### A.1.5 End-to-End Workflow

| ID | Test | Scenario | Expected | Status |
|----|------|----------|----------|--------|
| QA-22 | Vector E2E | Submit → Poll → Approve → Verify STAC | Full workflow completes | [ ] |
| QA-23 | Raster E2E | Submit → Poll → Approve → Verify STAC | Full workflow completes | [ ] |
| QA-24 | Overwrite E2E | Submit → Complete → Submit (overwrite) → Complete | Data replaced, URLs stable | [x] ✅ 29 JAN - Vector: new_hexagons table dropped & recreated, STAC preserved. Raster: tiny-raster2 checksum match → `no_change` in 4.1s, artifact preserved |
...
# QA-17: Unpublish (when done testing)
...
| PLT-03 | Platform lineage | GET /api/platform/lineage/{request_id} | Returns source→processing→output trace | [ ] |
...
|----|------|---------|----------|--------|
| INF-01 | Function App health | `curl $FA_URL/api/health` | `{"status": "healthy"}` | [x] ✅ 26 JAN |
| INF-02 | Docker Worker health | `curl $DW_URL/health` | `{"status": "healthy"}` | [x] ✅ 26 JAN |
| INF-03 | Docker Worker readiness | `curl $DW_URL/readyz` | `{"ready": true}` | [x] ✅ 26 JAN |
| INF-04 | Database connectivity | Health endpoint database component | PostgreSQL connected | [x] ✅ 26 JAN |

### B.1.2 Queue Infrastructure

| ID | Test | Command | Expected | Status |
|----|------|---------|----------|--------|
| INF-05 | container-tasks queue exists | Azure Portal or `az servicebus queue show` | Queue exists | [x] ✅ 26 JAN |
| INF-06 | functionapp-tasks queue exists | Azure Portal or `az servicebus queue show` | Queue exists | [x] ✅ 26 JAN |
| INF-07 | Docker listens to container-tasks | Check Docker health endpoint | `queue_name: container-tasks` | [x] ✅ 26 JAN |
| INF-08 | FA listens to functionapp-tasks | Check FA health endpoint | `queues_listening.functionapp_tasks: true` | [x] ✅ 26 JAN |
...
|----|------|-------|----------|--------|
| VEC-01 | Small KML ETL | roads.kml (0.35 MB) | Table created, STAC item | [x] ✅ 28 JAN - 483 rows imported, MULTILINESTRING geometry. Job `0b5d84dc...` |
| VEC-02 | Small Shapefile ETL | roads.zip (0.05 MB) | Table created, STAC item | [x] ✅ 26 JAN - 483 rows, MULTILINESTRING, Docker execution confirmed |
| VEC-03 | Small GeoPackage ETL | roads.gpkg (0.22 MB) | Table created, STAC item | [x] ✅ 26 JAN - 483 rows, MULTILINESTRING, Docker execution confirmed |
...
| VEC-06 | XLarge CSV ETL | acled_export.csv (1.3 GB) | Completes without timeout | [x] ✅ 27 JAN - 2,570,844 rows, 129 chunks, MULTIPOINT, ~16 min. **BUG-009 FIXED** (v0.7.35.4) |
...
| VEC-08 | LineString geometry | roads.kml (lines) | geometry_type = LINESTRING | [x] ✅ 26 JAN - MULTILINESTRING (promoted from LineString by geopandas) |
...
### B.3.1 Single COG Output (≤2GB threshold)

| ID | Test | Input | Expected | Status |
|----|------|-------|----------|--------|
| RAS-01 | Small raster COG | 25FEB02023857...P001.TIF (360 MB) | Single COG, `output_mode: "single_cog"` | [x] ✅ 27 JAN - Job `7715b1f2...`, 459 MB output, 68s processing |
| RAS-02 | Medium raster COG | 2021_Luang_Prabang_DTM.tif (986 MB) | Single COG, valid Cloud-Optimized | [x] ✅ 27 JAN - Job `4950fb40...`, 973 MB output, 243s, DEM auto-detected |
| RAS-03 | Large raster COG | 22FEB01105201...P001.TIF (1.3 GB) | Single COG (disk-based processing) | [x] ✅ 27 JAN - Job `c76b664e...`, 1698 MB output, 473s, disk-based processing, 8 bands |
| RAS-04 | STAC item created | Any output | Item in pgstac with correct metadata | [x] ✅ 27 JAN - Job `7715b1f2...`, `inserted_to_pgstac: true`, item_id: `small-raster-test-v8-v8-testing-v10` |
...
| RAS-07 | 10GB raster tiling | 2022DEC21WV2.tif (10.8 GB) | Tiled output, many tiles | [x] ✅ 27 JAN - Job `a604a350...`, 12 tiles (3×4 grid), 16.1 min, multi-instance parallelism confirmed |
...
| ID | Test | Validation | Expected | Status |
|----|------|------------|----------|--------|
| RAS-10 | Bands preserved | Compare input/output | Same band count | [x] ✅ 27 JAN - Job `7715b1f2...`, 8 bands in → 8 bands out |
| RAS-11 | CRS reprojection to 4326 | Check output CRS | **Always EPSG:4326** (doctrine) | [x] ✅ 27 JAN - BUG-003 fixed, verified with 3 test jobs |
| RAS-12 | NoData handled | Check output nodata | Preserved from input | [ ] |
| RAS-13 | TiTiler COG viewer | Single COG | `/cog/viewer?url=...` works | [~] ⚠️ PARTIAL - TiTiler URLs work. BUG-010/011 FIXED (pending deploy) - will add bidx for multi-band |
...
| APR-05 | Batch status lookup | GET /api/platform/approvals/status?stac_item_ids=a,b,c | Returns map of ID → approval status | [ ] Ready to test |
...
| ID | Test | Scenario | Expected | Status |
|----|------|----------|----------|--------|
| APR-12 | Full approval workflow | Submit raster → Create approval → Approve → Verify STAC | End-to-end approval flow works | [ ] Ready to test |
...
|------|---------|--------|-------|
| 00:17 UTC | INF-01 | ✅ PASS | FA healthy, all components OK |
| 00:17 UTC | INF-02 | ✅ PASS | Docker v0.7.32.5, GDAL 3.10.1, 7.7GB RAM |
| 00:17 UTC | INF-03 | ✅ PASS | Ready, tokens valid |
| 00:17 UTC | INF-04 | ✅ PASS | PostgreSQL 17.7, PostGIS 3.5, 201ms connect |
| 00:18 UTC | INF-05 | ✅ PASS | container-tasks queue active |
| 00:18 UTC | INF-06 | ✅ PASS | functionapp-tasks queue active |
| 00:18 UTC | INF-07 | ✅ PASS | Docker listening to container-tasks |
| 00:18 UTC | INF-08 | ✅ PASS | FA listening to functionapp-tasks |

### Session 2: 27 JAN 2026 (CRS Reprojection Fix)

| Time | Test ID | Result | Notes |
|------|---------|--------|-------|
| 00:00 UTC | RAS-11 | ❌ FAIL | Job `7715b1f2...` output EPSG:32750 instead of 4326. BUG-003 opened. |
| 00:30 UTC | - | 🔧 FIX | Identified root cause: `cog_translate` ignores `dst_crs` for file path inputs |
| 00:35 UTC | - | 🔧 FIX | First fix attempt: Added `dst_crs` to `cog_config` dict |
| 00:42 UTC | RAS-11 | ❌ FAIL | Retest still showed EPSG:32750 - `dst_crs` in config doesn't work for file paths |
| 00:45 UTC | - | 🔧 FIX | Researched rio-cogeo docs, found `WarpedVRT` is required for reprojection |
| 00:50 UTC | - | 🔧 FIX | Implemented WarpedVRT wrapper in `_process_cog_disk_based()` (v0.7.33.4) |
| 00:52 UTC | - | 🚀 DEPLOY | Built/deployed Docker image v0.7.33.4 |
| 01:00 UTC | RAS-11 | ✅ PASS | 3 jobs tested: EPSG:32750→4326 ✅, EPSG:32648→4326 ✅, EPSG:4326→4326 ✅ |

**Jobs Tested**:
- `7715b1f2107644c9...` - Source: EPSG:32750 (UTM 50S) → Output: EPSG:4326 ✅
- `4950fb4031cd6041...` - Source: EPSG:32648 (UTM 48N) → Output: EPSG:4326 ✅
- `d33b49613a01fa44...` - Source: EPSG:4326 (no-op) → Output: EPSG:4326 ✅

### Session 3: 27 JAN 2026 (Raster Testing & Multi-Instance Validation)

| Time | Test ID | Result | Notes |
|------|---------|--------|-------|
| 02:00 UTC | RAS-01 | ✅ PASS | Job `7715b1f2...`, 360 MB → 459 MB, 68s, single_cog |
| 02:00 UTC | RAS-04 | ✅ PASS | STAC item inserted to pgstac |
| 02:00 UTC | RAS-10 | ✅ PASS | 8 bands preserved |
| 02:00 UTC | RAS-13 | ✅ PASS | TiTiler viewer functional |
| 02:10 UTC | RAS-02 | ✅ PASS | Job `4950fb40...`, 986 MB DTM → 973 MB, 243s, DEM auto-detected |
| 02:31 UTC | - | 🧪 TEST | Submitted 10.8 GB (RAS-07) and 1.3 GB (RAS-03) jobs simultaneously |
| 02:35 UTC | - | 🎉 **CRITICAL** | **Multi-instance parallelism CONFIRMED** via App Insights logs |
| 02:40 UTC | RAS-03 | ✅ PASS | Job `c76b664e...`, 1.3 GB → 1698 MB, 473s, disk-based, 8 bands |

**🎉 Multi-Instance Parallelism Validation**:

Two Docker worker instances processed jobs **simultaneously**:
- Instance `e7c2c054ea58...` → Job `a604a350...` (10.8 GB) - TILED output, writing tiles
- Instance `50bd00d9b926...` → Job `c76b664e...` (1.3 GB) - Single COG processing

Evidence from Application Insights:
```
02:32:27 | 50bd00d9b926... | 🔧 Processing task (job: c76b664e..., 1.3GB)
02:32:49 | e7c2c054ea58... | Writing blob: ...tile_0_0.tif (10.8GB tiled job)
02:33:07 | e7c2c054ea58... | ✅ Wrote tile_0_0.tif (454 MB)
02:34:14 | e7c2c054ea58... | ✅ Wrote tile_0_1.tif (454 MB)
```

This validates the V0.8 Docker Worker horizontal scaling architecture.

**Single COG Section Complete**: RAS-01 ✅, RAS-02 ✅, RAS-03 ✅, RAS-04 ✅

---

## Test Data Preparation

### Test Data Verification (25 JAN 2026) ✅ COMPLETE
...
|------|------|--------|----------|
| `roads.kml` | 0.35 MB | KML | ✅ |
| `roads.zip` | 0.05 MB | Shapefile | ✅ |
| `roads.gpkg` | 0.22 MB | GeoPackage | ✅ |
| `acled_test.csv` | 2.83 MB | CSV | ✅ |
| `World_Bank_Global_Administrative_Divisions.geojson` | 136.81 MB | GeoJSON | ✅ |
| `acled_export.csv` | 1,333.60 MB | CSV | ✅ |

#### Raster Test Files

| File | Size | Category | Verified |
|------|------|----------|----------|
| `25FEB02023857-S2AS-200007488011_01_P001.TIF` | 359.73 MB | Small | ✅ |
| `2021_Luang_Prabang_DTM.tif` | 985.99 MB | Medium | ✅ |
| `22FEB01105201-S2AS_R1C1-200007559860_01_P001.TIF` | 1,318.56 MB | Large (disk) | ✅ |
| `17apr2024wv2.tif` | 4,449.54 MB | XLarge (tiled) | ✅ |
| `2024APR17WV2.tif` | 8,044.86 MB | XLarge (tiled) | ✅ |
| `2022DEC21WV2.tif` | 10,792.99 MB | XXLarge (tiled) | ✅ |
| `antigua.tif` | 11,161.39 MB | XXLarge (tiled) | ✅ |
...
| DB-23 | Platform request linked | `SELECT * FROM app.api_requests WHERE job_id='{id}'` | Request→job mapping | [ ] |
...
| ID | Test | Scenario | Expected | Status |
|----|------|----------|----------|--------|
| TSK-01 | Pending → Processing | Submit job, check task immediately | Task in 'pending' then 'processing' | [ ] |
| TSK-02 | Processing → Completed | Wait for job completion | Task status = 'completed' | [ ] |
| TSK-03 | Processing → Failed | Submit invalid job | Task status = 'failed', error_details populated | [ ] |
...
|--------|-------|---------|
| ✅ Fixed | 8 | BUG-001, BUG-003, BUG-004, BUG-005, BUG-007, BUG-008, BUG-010, BUG-011 |
| ✅ Closed (Not a Bug) | 2 | BUG-002, BUG-006 |
| ✅ Fixed & Deployed | 2 | BUG-009 (vector overwrite), BUG-012 (race condition) |

**Fixes Pending Deployment:**
- **BUG-009**: Vector `overwrite=true` now drops and recreates table (not append) ✅ DEPLOYED v0.7.35.4
- **BUG-012**: Overwrite race condition fixed - no async unpublish jobs, handler manages cleanup ✅ DEPLOYED v0.7.35.4
