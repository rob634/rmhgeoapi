...
class ArtifactRepository(PostgreSQLRepository):
    ...
    def _row_to_artifact(self, row: Dict[str, Any]) -> Artifact:
        ...
        return Artifact(
            ...
            supersedes=UUID(row['supersedes']) if isinstance(row.get('supersedes'), str) else row.get('supersedes'),
            superseded_by=UUID(row['superseded_by']) if isinstance(row.get('superseded_by'), str) else row.get('superseded_by'),
