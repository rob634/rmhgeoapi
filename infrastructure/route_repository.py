...
Provides upsert, delete, and lookup operations for the routing tables
used by the service layer to resolve slugs to PostGIS tables and STAC items.
...
class RouteRepository(PostgreSQLRepository):
    ...
    UPSERT_COLUMNS = (
        "slug", "version_id", "data_type", "version_ordinal",
    ...
    def get_by_slug(
        ...
        Args:
            slug: The route slug.
            version: Either 'latest' (default) to get the highest
                     version_ordinal, or a specific version_id (e.g. 'v1').
            table: Route table name.
