...
class ListContainerContentsDiamondWorkflow(JobBase):
    """
    TEST/DIAGNOSTIC TOOL - Three-stage diamond pattern for fan-in validation.

    ⚠️ THIS IS NOT A PRODUCTION WORKFLOW ⚠️
    Use "list_container_contents" for production container analysis.

    Purpose:
    - Validates CoreMachine's fan-in aggregation pattern
    - Demonstrates diamond pattern (1→N→1) with auto-aggregation
    - Tests Stage 3 auto-creation by CoreMachine
    ...
    Testing: See docs/archive/completed/DIAMOND_PATTERN_TEST.md
    """
    ...
    def finalize_job(context=None) -> Dict[str, Any]:
        """
        Create final job summary (minimal pattern - diagnostic workflow).
        ...
        logger = LoggerFactory.create_logger(ComponentType.CONTROLLER, "ListContainerContentsDiamondWorkflow.finalize_job")

        if context:
            logger.info(f"✅ Job {context.job_id} completed with {len(context.task_results)} tasks")
        else:
            logger.info("✅ ListContainerContentsDiamond job completed")
