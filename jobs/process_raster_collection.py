...
class ProcessRasterCollectionWorkflow(JobBase):
    ...
    @staticmethod
    def _create_stage_3_tasks(
        job_id: str,
        job_params: Dict[str, Any],
        previous_results: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        Stage 3: Create MosaicJSON task (fan-in).

        Args:
            job_id: Job ID
            job_params: Validated job parameters
            previous_results: COG creation results from Stage 2

        Returns:
            List with single create_mosaicjson task

        Raises:
            ValueError: If COG creation failed or create_mosaicjson disabled
        """
        # Check if MosaicJSON creation is enabled
        if not job_params.get("create_mosaicjson", True):
            return []  # Skip stage if disabled

        # Check that all COG creations succeeded
        failed_cogs = [
            r for r in previous_results
            if not r.get("result_data", {}).get("success", False)
        ]
        if failed_cogs:
            raise ValueError(
                f"{len(failed_cogs)} COG creations failed. "
                f"Cannot proceed to MosaicJSON creation."
            )

        # Extract COG blob paths from Stage 2 results
        cog_blobs = [
        ...
        if not cog_blobs:
            raise ValueError("No COG blob paths found in Stage 2 results")
        ...
        task = {
            ...
            "parameters": {
                "cog_blobs": cog_blobs,
                "collection_name": job_params["collection_id"],
                "container": "rmhazuregeosilver",
                "output_folder": job_params.get("output_folder", "mosaics")
            },
            "metadata": {
    ...
    @staticmethod
    def _create_stage_4_tasks(
        job_id: str,
        job_params: Dict[str, Any],
        previous_results: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        Stage 4: Create STAC collection task (fan-in).

        Args:
            job_id: Job ID
            job_params: Validated job parameters
            previous_results: MosaicJSON creation result from Stage 3

        Returns:
            List with single create_stac_collection task

        Raises:
            ValueError: If MosaicJSON creation failed or create_stac_collection disabled
        """
        # Check if STAC collection creation is enabled
        if not job_params.get("create_stac_collection", True):
            return []  # Skip stage if disabled

        # Check that MosaicJSON creation succeeded
        ...
        if not mosaic_result.get("success", False):
            raise ValueError("MosaicJSON creation failed. Cannot create STAC collection.")

        mosaicjson_blob = mosaic_result.get("mosaicjson_blob")
        if not mosaicjson_blob:
            raise ValueError("No mosaicjson_blob found in Stage 3 result")

        # Get COG blobs from job metadata (stored in Stage 2)
        # We need to look back at the job record to get Stage 2 results
        # For now, extract from mosaic_result if available
        cog_blobs = mosaic_result.get("cog_blobs", [])

        # Extract spatial extent from MosaicJSON result
        ...
        task = {
            ...
            "parameters": {
                ...
                "description": job_params.get("collection_description"),
                "tile_blobs": cog_blobs,
                "container": "rmhazuregeosilver",
                "license": "proprietary",
                "spatial_extent": spatial_extent,
                "temporal_extent": None  # Auto-generated as current time
            },
            "metadata": {
                "collection_id": job_params["collection_id"],
                "mosaicjson_blob": mosaicjson_blob
