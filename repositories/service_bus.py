...
class ServiceBusRepository(IQueueRepository):
    ...
    def _get_receiver(self, queue_or_topic: str) -> ServiceBusReceiver:
        """Get or create a message receiver."""
        if queue_or_topic not in self._receivers:
            logger.debug(f"🚌 Creating receiver for: {queue_or_topic}")
            self._receivers[queue_or_topic] = self.client.get_queue_receiver(queue_or_topic)
        return self._receivers[queue_or_topic]
