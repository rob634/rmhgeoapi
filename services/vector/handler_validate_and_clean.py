...
def vector_validate_and_clean(params: Dict[str, Any], context: Optional[Any] = None) -> Dict[str, Any]:
    ...
    try:  # Outer try/except — ensures handler contract on unexpected errors
        # ------------------------------------------------------------------
        # 2. SOURCE FILE CHECK
        # ------------------------------------------------------------------
        ...
        import traceback
        logger.error(f"{log_prefix} Unexpected error: {exc}\n{traceback.format_exc()}")
        return {
            "success": False,
            "error": f"Unexpected error in vector_validate_and_clean: {exc}",
    ...
    def fix_one(geom):
        ...
        if maxx > 180:
            ...
            try:
                ...
                logger.debug(f"{log_prefix} Antimeridian split failed (maxx>180): {exc}")
                return geom, False
        ...
        if width > 180:
            ...
            try:
                ...
                logger.debug(f"{log_prefix} Antimeridian split failed (width>180): {exc}")
                return geom, False
