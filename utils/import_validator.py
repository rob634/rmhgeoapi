...
class ImportValidator:
    ...
    **2. Application Modules (Auto-Discovered):**
        ...
        - service_*.py, model_*.py, repository_*.py, trigger_*.py
        - util_*.py, validator_*.py, schema_*.py, adapter_*.py
    ...
    def _detect_azure_functions_environment(self) -> bool:
        ...
        if detected:
            self.logger.debug("Azure Functions environment detected via environment variables")
        else:
            self.logger.debug("Local development environment detected")
    ...
    def validate_all_imports(self, force_recheck: bool = False) -> Dict[str, Any]:
        ...
        # Log summary
        status_icon = "SUCCESS" if validation_result['success'] else "FAILED"
        failed_count = len(critical_result['failed']) + len(app_result['failed'])
        warning_count = len(validation_result['warnings'])
        discovered_count = len(discovered_modules)

        self.logger.info(f"Import validation {status_icon} - Failed: {failed_count}, Warnings: {warning_count}, Auto-discovered: {discovered_count}")
    ...
    def _validate_critical_imports(self) -> Dict[str, Any]:
        ...
        """
        self.logger.debug("Validating critical imports")
        ...
        for module_name, description in critical_modules:
            try:
                ...
                self.logger.debug(f"OK: {module_name} - {description}")
    ...
    def ensure_startup_ready(self) -> None:
        ...
        if not (self.is_azure_functions or self.force_validation):
            self.logger.debug("Startup validation skipped - not in Azure Functions environment")
            return

        self.logger.info("Starting fail-fast startup validation")
        ...
        self.logger.info("Startup validation successful - all critical dependencies available")
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.debug("Generating health status report")
            ...
            self.logger.info(f"Health status generated - Status: {status}")
